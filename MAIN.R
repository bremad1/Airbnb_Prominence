...
tex_file <- if (length(args) >= 1L) args[[1L]] else
  "2_first_month_ltm_ge1_results.tex"
...
apply_filters <- function(data, change_pct) {
  ...
  # The requested additional restriction.
  x %>% filter(!is.na(first_month_ltm), first_month_ltm >= 1)
}
...
make_overview <- function(results) {
  ...
  lines <- c(
    ...
    "\\caption{FULL results with \\texttt{first\\_month\\_ltm $\\geq 1$}}",
...
make_detail_table <- function(d, price_trim, change_trim) {
  ...
  c(
    ...
    paste0(
      ...
      "$\\geq 1$}. Panel A is defined by \\texttt{ex\\_super==\\\"t\\\"}; ",
...
tex_lines <- c(
  "\\section{\\texttt{first\\_month\\_ltm} $\\geq 1$}",
