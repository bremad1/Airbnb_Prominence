...
for (panel_type in c("t", "f")) {
  ...
  panel_keep <- z$ex_super == panel_type
...
write.csv(result, file.path(out_dir, "first_stage_no_exsuper2_rdrobust300.csv"), row.names = FALSE)
write.csv(count_result, file.path(out_dir, "first_stage_no_exsuper2_counts.csv"), row.names = FALSE)
...
writeLines(tex_lines, file.path(out_dir, "first_stage_no_exsuper2_rdrobust300.tex"))
