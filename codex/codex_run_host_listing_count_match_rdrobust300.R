...
z <- bind_rows(Q323 = Q323, Q423 = Q423, Q124 = Q124, Q224 = Q224, Q324 = Q324, Q424 = Q424, .id = "quarter")
  ...
  for (condition_name in names(conditions)) {
    ...
    for (filter_id in names(filters)) {
      ...
      counts[[count_key]] <- data.frame(
        ...
        kept_match_host_listings = sum(keep & match_host_listing_count, na.rm = TRUE),
        kept_match_host_total_listings = sum(keep & match_host_total_listing_count, na.rm = TRUE),
        stringsAsFactors = FALSE
      )
...
write.csv(result, file.path(out_dir, "host_listing_count_match_rdrobust300.csv"), row.names = FALSE)
write.csv(count_result, file.path(out_dir, "host_listing_count_match_counts.csv"), row.names = FALSE)
...
writeLines(tex_lines, file.path(out_dir, "host_listing_count_match_rdrobust300.tex"))
