...
source_q <- lapply(seq_len(nrow(quarter_map)), function(i) {
  qm <- quarter_map[i, ]
  entire_dates %>%
    ...
    summarise(
      quarter = qm$quarter,
      any_previous_scrape_q = any(source == "previous scrape", na.rm = TRUE),
      all_city_scrape_q = all(source == "city scrape"),
      previous_scrape_months_q = sum(source == "previous scrape", na.rm = TRUE),
      source_months_q = n_distinct(.month[!is.na(source)]),
      .groups = "drop"
    )
}) %>%
  bind_rows()
...
z <- bind_rows(lapply(names(quarter_list), function(qname) {
  ...
  left_join(source_q, by = c("id", "quarter"))
...
filters <- list(
  ...
  drop_row_previous_scrape = list(
    label = "drop row source=previous scrape",
    keep = is.na(z$source) | z$source != "previous scrape"
  ),
  drop_any_target_previous_scrape = list(
    label = "drop any target-month source=previous scrape",
    keep = is.na(z$any_previous_scrape_q) | !z$any_previous_scrape_q
  )
)
  ...
  for (condition_name in names(conditions)) {
    ...
    for (filter_id in names(filters)) {
      ...
      counts[[count_key]] <- data.frame(
        ...
        dropped_row_previous_scrape = sum(dropped & z$source == "previous scrape", na.rm = TRUE),
        dropped_any_previous_scrape_q = sum(dropped & z$any_previous_scrape_q, na.rm = TRUE),
        stringsAsFactors = FALSE
      )
...
write.csv(result, file.path(out_dir, "drop_previous_scrape_exsuper2_rdrobust300.csv"), row.names = FALSE)
write.csv(count_result, file.path(out_dir, "drop_previous_scrape_exsuper2_counts.csv"), row.names = FALSE)
...
for (panel_id in c("panel_a", "panel_b")) {
  for (condition_name in names(conditions)) {
    for (filter_id in names(filters)) {
      ...
      tex_lines <- c(
        tex_lines,
        paste(c(tex_escape(panel_id), condition_name, paste0(tex_escape(filters[[filter_id]]$label), " (N=", count_row$kept_n, ", drop=", count_row$dropped_n, ")"), coefs), collapse = " & "),
        "\\\\",
...
writeLines(tex_lines, file.path(out_dir, "drop_previous_scrape_exsuper2_rdrobust300.tex"))

cat("rdrobust", as.character(packageVersion("rdrobust")), "\n")
cat("\nCounts:\n")
print(count_result)
cat("\nResults:\n")
print(result[, c("panel", "condition", "filter_id", "raw_n", "spec", "coef_conv", "se_conv", "pv_conv", "coef_bc", "pv_robust", "obs_h", "error")], row.names = FALSE)
