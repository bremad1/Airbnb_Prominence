...
host_ltm <- z %>%
  distinct(quarter, host_id, id, scraped_host_ltm_count, ltm_scr, number_of_reviews_ltm) %>%
  ...
  summarise(
    ...
    host_inside_current_ltm = ifelse(
      all(is.na(number_of_reviews_ltm)),
      NA_real_,
      sum(number_of_reviews_ltm, na.rm = TRUE)
    ),
    inside_current_ltm_missing_n = sum(is.na(number_of_reviews_ltm)),
    .groups = "drop"
  ) %>%
  mutate(
    diff_current_ltm = host_scraped_ltm_count - host_inside_current_ltm,
    abs_diff_current_ltm = abs(diff_current_ltm),
    current_ltm_exact = !is.na(host_scraped_ltm_count) &
      !is.na(host_inside_current_ltm) &
      inside_current_ltm_missing_n == 0 &
      scraped_unique_n == 1 &
      abs_diff_current_ltm == 0,
    current_ltm_within2 = !is.na(host_scraped_ltm_count) &
      !is.na(host_inside_current_ltm) &
      inside_current_ltm_missing_n == 0 &
      scraped_unique_n == 1 &
      abs_diff_current_ltm <= 2,
    single_listing_current_ltm_within2 = host_n_listings_q == 1 & current_ltm_within2
  )

z <- z %>% left_join(host_ltm, by = c("quarter", "host_id"))
...
filters <- list(
  ...
  current_ltm_within2 = list(
    label = "|ltm_scr - host sum number_of_reviews_ltm| <= 2",
    keep = z$current_ltm_within2
  ),
  single_listing_current_ltm_within2 = list(
    label = "single-listing host and |ltm_scr - number_of_reviews_ltm| <= 2",
    keep = z$single_listing_current_ltm_within2
  )
)
...
diagnostics <- bind_rows(lapply(names(samples), function(sample_id) {
  ...
  bind_rows(lapply(names(filters), function(filter_id) {
    ...
    data.frame(
      ...
      mean_diff_current_ltm = mean(data$diff_current_ltm, na.rm = TRUE),
      median_diff_current_ltm = median(data$diff_current_ltm, na.rm = TRUE),
      mean_abs_diff_current_ltm = mean(data$abs_diff_current_ltm, na.rm = TRUE),
      median_abs_diff_current_ltm = median(data$abs_diff_current_ltm, na.rm = TRUE),
      mean_scraped = mean(data$host_scraped_ltm_count, na.rm = TRUE),
      mean_inside_current_ltm = mean(data$host_inside_current_ltm, na.rm = TRUE),
      stringsAsFactors = FALSE
    )
  }))
}))

write.csv(host_ltm, file.path(out_dir, "ltm_scr_current_ltm_host_panel.csv"), row.names = FALSE)
write.csv(diagnostics, file.path(out_dir, "ltm_scr_current_ltm_within2_counts.csv"), row.names = FALSE)
...
write.csv(result, file.path(out_dir, "ltm_scr_current_ltm_within2_rdrobust300.csv"), row.names = FALSE)
...
make_tex("price_conv", "price_se_conv", "price_pv_conv", "ltm_scr_current_ltm_within2_price_rdrobust300.tex")
make_tex("fs_conv", "fs_se_conv", "fs_pv_conv", "ltm_scr_current_ltm_within2_first_stage_rdrobust300.tex")
