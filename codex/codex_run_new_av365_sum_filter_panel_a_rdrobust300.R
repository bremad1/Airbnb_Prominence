...
availability_sums <- lapply(seq_len(nrow(quarter_map)), function(i) {
  ...
  entire_dates %>%
    ...
    summarise(
      ...
      availability_365_zero_months_q = sum(availability_365 == 0, na.rm = TRUE),
      .groups = "drop"
    )
}) %>%
  bind_rows()
...
samples <- list(
  panel_a_ex2 = list(
    label = "ex_super=t & ex_super2=t",
    keep = z$ex_super == "t" & z$ex_super2 == "t" & full_condition
  ),
  panel_a_ex2_verified = list(
    label = "ex_super=t & ex_super2=t & host_identity_verified=t",
    keep = z$ex_super == "t" & z$ex_super2 == "t" & z$host_identity_verified == "t" & full_condition
  )
)
...
filters <- list(
  ...
  drop_av365_row0 = list(
    label = "drop row availability_365=0",
    fn = !(z$availability_365 == 0)
  ),
  drop_av365_sum0 = list(
    label = "drop quarter-sum availability_365=0",
    fn = !(z$availability_365_sum_q == 0)
  )
)
...
for (sample_id in names(samples)) {
  ...
  for (filter_id in names(filters)) {
    ...
    counts[[paste(sample_id, filter_id, sep = "__")]] <- data.frame(
      ...
      dropped_row_av3650 = sum(dropped & z$availability_365 == 0, na.rm = TRUE),
      dropped_sum_av3650 = sum(dropped & z$availability_365_sum_q == 0, na.rm = TRUE),
      kept_mean_av365_sum_q = mean(data$availability_365_sum_q, na.rm = TRUE),
      dropped_mean_av365_sum_q = mean(z$availability_365_sum_q[dropped], na.rm = TRUE),
      stringsAsFactors = FALSE
    )
...
write.csv(result, file.path(out_dir, "new_av365_sum_filter_panel_a_rdrobust300.csv"), row.names = FALSE)
write.csv(count_result, file.path(out_dir, "new_av365_sum_filter_panel_a_counts.csv"), row.names = FALSE)
...
writeLines(tex_lines, file.path(out_dir, "new_av365_sum_filter_panel_a_rdrobust300.tex"))

cat("rdrobust", as.character(packageVersion("rdrobust")), "\n")
cat("\nCounts:\n")
print(count_result)
cat("\nResults:\n")
print(result[, c("sample_id", "filter_id", "raw_n_panel_a", "spec", "coef_conv", "se_conv", "pv_conv", "coef_bc", "pv_robust", "obs_h", "error")], row.names = FALSE)
