...
quarter_review_diff <- bind_rows(lapply(seq_len(nrow(quarter_map)), function(i) {
  ...
  current <- entire_reviews %>%
    ...
    transmute(id, current_first_month_number_of_reviews = number_of_reviews_snapshot)
  previous <- entire_reviews %>%
    filter(ym == qm$previous_ym) %>%
    transmute(id, previous_first_month_number_of_reviews = number_of_reviews_snapshot)
  ...
  full_join(current, previous, by = "id") %>%
    mutate(
      ...
      listing_first_month_review_diff = current_first_month_number_of_reviews - previous_first_month_number_of_reviews
    )
}))
...
host_diff <- z %>%
  distinct(
    ...
    host_id,
    id,
    listing_first_month_review_diff,
    scraped_host_ltm_count
  ) %>%
  group_by(quarter, host_id) %>%
  summarise(
    host_n_listings_q = n_distinct(id),
    host_first_month_review_diff = ifelse(
      all(is.na(listing_first_month_review_diff)),
      NA_real_,
      sum(listing_first_month_review_diff, na.rm = TRUE)
    ),
    host_diff_missing_n = sum(is.na(listing_first_month_review_diff)),
    host_diff_negative_n = sum(listing_first_month_review_diff < 0, na.rm = TRUE),
    scraped_host_ltm_count_unique_n = n_distinct(scraped_host_ltm_count[!is.na(scraped_host_ltm_count)]),
    host_scraped_ltm_count = ifelse(
      scraped_host_ltm_count_unique_n == 1,
      first(scraped_host_ltm_count[!is.na(scraped_host_ltm_count)]),
      NA_real_
    ),
    .groups = "drop"
  ) %>%
  mutate(
    host_first_month_diff_match = !is.na(host_scraped_ltm_count) &
      !is.na(host_first_month_review_diff) &
      host_diff_missing_n == 0 &
      scraped_host_ltm_count_unique_n == 1 &
      host_scraped_ltm_count == host_first_month_review_diff,
    host_first_month_diff_match_nonnegative = host_first_month_diff_match &
      host_diff_negative_n == 0
  )

z <- z %>%
  left_join(host_diff, by = c("quarter", "host_id")) %>%
  mutate(
    row_first_month_diff_match = !is.na(scraped_host_ltm_count) &
      !is.na(listing_first_month_review_diff) &
      scraped_host_ltm_count == listing_first_month_review_diff,
    single_listing_first_month_diff_match = host_n_listings_q == 1 &
      row_first_month_diff_match,
    existing_first_month_mismatch = !is.na(first_month_number_of_reviews) &
      !is.na(current_first_month_number_of_reviews) &
      first_month_number_of_reviews != current_first_month_number_of_reviews
  )
...
mapping_audit <- z %>%
  ...
  summarise(
    rows = n(),
    ltm_scr_mapped_count_mismatch = sum(ltm_scr != scraped_host_ltm_count, na.rm = TRUE),
    existing_first_month_mismatch = sum(existing_first_month_mismatch, na.rm = TRUE),
    missing_current_first_month = sum(is.na(current_first_month_number_of_reviews)),
    missing_previous_first_month = sum(is.na(previous_first_month_number_of_reviews)),
    missing_listing_diff = sum(is.na(listing_first_month_review_diff)),
    negative_listing_diff = sum(listing_first_month_review_diff < 0, na.rm = TRUE),
    host_diff_match_rows = sum(host_first_month_diff_match, na.rm = TRUE),
    row_diff_match_rows = sum(row_first_month_diff_match, na.rm = TRUE),
    single_listing_diff_match_rows = sum(single_listing_first_month_diff_match, na.rm = TRUE),
    .groups = "drop"
  )

write.csv(mapping_audit, file.path(out_dir, "first_month_cumulative_diff_mapping_audit.csv"), row.names = FALSE)
write.csv(host_diff, file.path(out_dir, "first_month_cumulative_diff_host_panel.csv"), row.names = FALSE)
...
filters <- list(
  ...
  host_first_month_diff_match = list(
    label = "host scraped count = host sum first-month cumulative review diff",
    keep = z$host_first_month_diff_match
  ),
  host_first_month_diff_match_nonnegative = list(
    label = "same, requiring no negative listing diffs",
    keep = z$host_first_month_diff_match_nonnegative
  ),
  row_first_month_diff_match = list(
    label = "row first-month cumulative diff = host scraped count",
    keep = z$row_first_month_diff_match
  ),
  single_listing_first_month_diff_match = list(
    label = "single-listing host and first-month cumulative diff match",
    keep = z$single_listing_first_month_diff_match
  )
)
...
diagnostics <- bind_rows(lapply(names(samples), function(sample_id) {
  ...
  bind_rows(lapply(names(filters), function(filter_id) {
    ...
    data.frame(
      ...
      mean_listing_diff = mean(data$listing_first_month_review_diff, na.rm = TRUE),
      mean_host_diff = mean(data$host_first_month_review_diff, na.rm = TRUE),
      mean_scraped_count = mean(data$scraped_host_ltm_count, na.rm = TRUE),
      negative_listing_diff_rows = sum(data$listing_first_month_review_diff < 0, na.rm = TRUE),
      stringsAsFactors = FALSE
    )
  }))
}))

write.csv(diagnostics, file.path(out_dir, "first_month_cumulative_diff_match_counts.csv"), row.names = FALSE)
...
write.csv(result, file.path(out_dir, "first_month_cumulative_diff_match_rdrobust300.csv"), row.names = FALSE)
...
make_tex("price_conv", "price_se_conv", "price_pv_conv", "first_month_cumulative_diff_match_price_rdrobust300.tex")
make_tex("fs_conv", "fs_se_conv", "fs_pv_conv", "first_month_cumulative_diff_match_first_stage_rdrobust300.tex")
