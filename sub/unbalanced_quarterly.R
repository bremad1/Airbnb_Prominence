...
# This is the UNBALANCED companion to balanced_quarterly.R: a listing does
# NOT need to appear in all six target quarters. Each listing-quarter's
# price_diff is instead computed against that same listing's immediately
# preceding quarter via lag() over its full observed history (i.e. it only
# ever looks at the six months spanning a quarter and the one before it).
# If a listing has no valid immediately-preceding quarter, price_diff is NA
# for that row and it is dropped later by the review/trim eligibility
# filter -- only that one listing-quarter drops, not the whole listing.
#
# Order of operations:
...
# File names are prefixed "unbalanced_quarterly_" so they never collide
# with balanced_quarterly.R's outputs if both are run in the same directory.
...
build_quarter_panel <- function(monthly_data) {
  ...
  quarterly <- monthly %>%
    ...
    # A complete listing-quarter requires all three calendar months, with
    # host_is_superhost observed and identical throughout (matches the
    # balanced companion script's id-quarter requirement).
    filter(
    ...
    mutate(
      # Deliberately use only the last two months of the quarter (matches
      # the balanced companion script).
  ...
  quarterly <- quarterly %>%
    # Same host-quarter status must also agree across all listings
    # belonging to that host (matches the balanced companion script's
    # host_id-level requirement -- full non-missing agreement, not just
    # agreement among whichever months happen to be non-missing).
    ...
    ungroup() %>%
    # Restrict to the six target quarters only now, after price_diff has
    # been computed against each listing's full observed history -- doing
    # this earlier would sever legitimate lag links (e.g. Q323's price_diff
    # depends on a Q223 row that is itself outside the target window).
    filter(quarter %in% target_quarters) %>%
    # Match the balanced companion script's requirement that the RD
    # variables used in fit_main_rd() are present. Applied here (after
    # price_diff/lag), not inside the id-quarter block above, so it can't
    # break lag continuity for other listing-quarters.
    filter(
  ...
  stopifnot(
    ...
    # Note: price_diff is intentionally allowed to be NA here (unbalanced
    # design) whenever a listing-quarter has no valid immediately-preceding
    # quarter. eligible <- ... %>% filter(is.finite(price_diff)) in
    # run_analysis() drops those rows later.
  )
...
fit_main_rd <- function(data) {
  ...
  # Defensive re-filter, matching the balanced companion script -- upstream
  # filtering should already guarantee this, but rdrobust fails opaquely on
  # NA/non-finite inputs, so check explicitly here too.
  data <- data %>%
    filter(
      is.finite(price_diff),
      is.finite(running_scr),
  ...
  rd_args <- list(
    ...
    covs = as.matrix(time_dummies[, -1L, drop = FALSE]),
    cluster = data$id,
    kernel = "tri",
    bwselect = "msetwo",
    p = 1,
...
review_threshold_panel_table <- function(
  ...
  notes <- paste0(
    "Listings need not appear in all six target quarters (unbalanced panel); ",
    "each listing-quarter's price change is measured against that same ",
    "listing's own immediately preceding quarter. Before trimming, the ",
    "sample is restricted to \\texttt{first\\_month\\_ltm} $\\geq 1$ and ",
    "\\texttt{first\\_month\\_number\\_of\\_reviews} at least the threshold ",
    "shown. Column headings report (price trim, price-change trim). ",
    ex_super_note, " ",
  ...
  paste(
    c(
      ...
      "\\centering",
      sprintf(
        "\\caption{RD estimates by initial cumulative review threshold (unbalanced panel): Panel %s}",
        panel_letter
      ),
      sprintf(
        "\\label{tab:unbalanced_review_threshold_0to50_panel_%s}",
...
run_analysis <- function(
    results_file =
      "results/unbalanced_quarterly_review_threshold_panels.tex",
    data_file =
      "results/unbalanced_quarterly_active_review_trim_data.rds",
    audit_file =
      "results/unbalanced_quarterly_active_review_trim_audit.tex",
    review_seq = seq(0L, 50L, by = 5L)
) {
  
  cat("=== [run_analysis] building unbalanced quarterly panel ===\n")
  ...
  writeLines(
    df_to_latex(
      trim_audit,
      caption = "Quarter-level trim audit (unbalanced panel)",
      label = "tab:unbalanced_quarterly_trim_audit"
    ),
    audit_file
  )
  saveRDS(
