# generate_quarterly_dataset : all (must exists for all 6 analysis_months) + first_month_ltm>=1
# generate_quarterly_dataset2 : all (must exists for all 6 analysis_months) 
# generate_quarterly_dataset3 : any (at least once exists among 6 analysis_months) + first_month_ltm>=1
# generate_quarterly_dataset4 : any (at least once exists among 6 analysis_months)
...
generate_quarterly_dataset2 <- function(data, valid_months, analysis_months, target_year, target_months, cross_year = FALSE, pct = 0.05) {
  # Step 1: no_dup
  no_dup <- data %>%
    group_by(id) %>%
    filter(n_distinct(host_id) == 1) %>%
...
generate_quarterly_dataset3 <- function(data, valid_months, analysis_months, target_year, target_months, cross_year = FALSE, pct = 0.05) {
  # Step 1: no_dup
  no_dup <- data %>%
    group_by(id) %>%
    filter(n_distinct(host_id) == 1) %>%
  ...
  if (cross_year==F){
    ...
    temp_data <- no_dup %>%
      ...
      filter(host_is_superhost != '' & !is.na(host_is_superhost) & !is.na(price)) %>%
      group_by(id) %>%
      filter(any(analysis_months %in% month(Date))) %>%
      ungroup() %>%
      filter(
        ( (year(Date) == target_year & month(Date) %in% target_months)
        )
...
generate_quarterly_dataset4 <- function(data, valid_months, analysis_months, target_year, target_months, cross_year = FALSE, pct = 0.05) {
  # Step 1: no_dup
  no_dup <- data %>%
    group_by(id) %>%
    filter(n_distinct(host_id) == 1) %>%
