...
runrd <-function(data,level) {
  ...
  for (super_type in c('f', 't')) {
    for (condition_name in names(conditions)) {
      ...
      filtered_data <- z %>%
        filter(ex_super == super_type & condition & ex_super2=='t')
...
save_table2 = function(detail,robust) {
  tex_data <-data.frame()
  tex_data <- data.frame(
  ...
  tex_data <- tex_data %>%
    ...
    arrange(Ex_super)
  table_name <- "\\begin{table}[]\n \\TABLE \n\ {EX2\\_Entire Room Regression Results(Ex==1) \\label{ex2_result_detail}} {\\begin{tabular}{lcccccc}\n"
  ...
  if (detail == TRUE & robust == TRUE) {
    table = "tex/6-1-1. Ex2_Entire_detail_robust.tex"
  } else if (detail == TRUE & robust == FALSE) {
    table = "tex/6-2-1. Ex2_Entire_detail_conv.tex"
  } else if (detail == FALSE & robust == TRUE) {
    table = "tex/Ex_2_Ent_robust.tex"
  } else if (detail == FALSE & robust == FALSE) {
    table = "tex/Ex_2_Ent_conv.tex"
  }
