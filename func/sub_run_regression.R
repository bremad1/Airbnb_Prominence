REGRESSION_PROJECT_DIR <- if (file.exists(file.path(
  getwd(), "func", "sub_run_regression.R"
))) {
  normalizePath(getwd(), winslash = "/", mustWork = TRUE)
} else if (file.exists(file.path(
  getwd(), "Final2", "func", "sub_run_regression.R"
))) {
  normalizePath(file.path(getwd(), "Final2"), winslash = "/", mustWork = TRUE)
} else {
  stop("Run sub_run_regression() from the project root or Final2 directory.")
}
...
REGRESSION_HELPERS <- load_balanced_3month_helpers(force = TRUE)
required_regression_helpers <- c(
  "prepare_runrd_sample",
  "runrd_subpanel_conditions"
)
missing_regression_helpers <- required_regression_helpers[
  !vapply(
    required_regression_helpers,
    exists,
    logical(1L),
    envir = REGRESSION_HELPERS,
    inherits = FALSE,
    mode = "function"
  )
]
if (length(missing_regression_helpers) > 0L) {
  stop(
    "Missing balanced three-month regression helper(s): ",
    paste(missing_regression_helpers, collapse = ", ")
  )
}
...
make_subpanel_table <- function(
  ...
  review_note <- if (is.null(review_min)) {
    paste0(
      "does not impose a ",
    ...
    paste0(
      "imposes \\texttt{first\\_month\\_number\\_of\\_reviews}",
      "$\\geq", as.integer(review_min), "$ before trimming, "
    )
  }
  ...
  paste(
    c(
      ...
      "\\footnotesize",
      paste0(
        "\\textit{Notes:} The sample uses trim (1,0), ",
        review_note,
...
sub_run_regression <- function(refresh_data = FALSE) {
