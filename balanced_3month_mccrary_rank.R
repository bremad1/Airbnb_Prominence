...
# Reuse exactly the balanced three-month quarter construction and trimming
# rules. source() does not execute balanced-3month.R's command-line block.
source("balanced-3month.R")

TRUE_CUTOFF <- 4.75
CANDIDATE_CUTOFFS <- seq(4.45, 4.95, by = 0.01)
OUTPUT_DIR <- "results/balanced_3month_mccrary_rank"
...
run_true_cutoff_density <- function(
  ...
  if (!is.null(density_plot$Estplot) && save_figure) {
    ...
    ggsave(
      file.path(output_dir, "rddensity_4_75.eps"),
...
run_mccrary_rank <- function(
  ...
  quarter_listing <- build_quarter_panel(Entire) %>%
    filter(
  ...
  t_plot <- ggplot(
    scan %>% filter(eligible_for_rank, is.finite(t_jk)),
    aes(x = cutoff, y = t_jk)
  ) +
    geom_hline(yintercept = 0, color = "grey45", linewidth = 0.4) +
    geom_hline(
      yintercept = c(-qnorm(0.975), qnorm(0.975)),
      linetype = "dashed",
      color = "#B2182B",
      linewidth = 0.5
    ) +
    geom_line(color = "#2166AC", linewidth = 0.55) +
    geom_point(color = "#2166AC", size = 1.4) +
    geom_point(
      data = scan %>% filter(
        bandwidth_contains_true_cutoff,
        is.finite(t_jk)
    ...
    annotate(
      ...
      x = TRUE_CUTOFF,
      y = true_result$t_jk,
      label = true_rank_label,
      hjust = if (TRUE_CUTOFF > mean(range(scan$cutoff))) 1.05 else -0.05,
      vjust = -0.7,
      size = 3.3
    ) +
    labs(
      title = "McCrary density test across candidate cutoffs",
      subtitle = "Dashed lines are the two-sided 5% critical values (±1.96)",
      x = "Candidate cutoff",
      y = "Jackknife T-statistic"
    ) +
  ...
  if (save_figures) {
    ggsave(
      file.path(output_dir, "mccrary_t_stat_by_cutoff.eps"),
      t_plot,
      device = grDevices::cairo_ps,
      width = 9,
      height = 5.5,
    ...
    ggsave(
      file.path(output_dir, "density_jump_ci_by_cutoff.eps"),
      jump_plot,
      device = grDevices::cairo_ps,
      width = 9,
      height = 5.5,
...
if (sys.nframe() == 0L) {
  ...
  run_mccrary_rank(output_dir = output_dir)
}
