FINAL_SCRIPT_NAME <- "run_balanced_3month_final_review_tables.R"
FINAL_DIR <- if (file.exists(file.path(
  getwd(), "Final", FINAL_SCRIPT_NAME
))) {
...
FINAL_RESULTS_DIR <- file.path(FINAL_DIR, "results")

CALLING_WORKING_DIR <- getwd()
setwd(PROJECT_ROOT)
tryCatch(
  source(file.path(
    PROJECT_ROOT,
    "Best script",
    "balanced-3month-runrd.R"
  )),
  finally = setwd(CALLING_WORKING_DIR)
)
