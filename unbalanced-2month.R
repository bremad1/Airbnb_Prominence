...
review_threshold_panel_table <- function(
  ...
  notes <- paste0(
    ...
    "months. Before trimming, the sample is restricted to \\texttt{first\\_month\\_ltm} ",
  ...
  paste(
    c(
      ...
      "\\centering",
      sprintf(
        "\\caption{RD estimates by initial cumulative review threshold (2-month price window): Panel %s (%s)}",
        panel_letter,
        cluster_display_name(cluster_type)
      ),
      sprintf(
        "\\label{tab:unbalanced_2month_review_threshold_0to50_%s_panel_%s}",
        cluster_label,
...
run_analysis <- function(
    results_file =
      "results/unbalanced_2month_four_cluster_review_threshold_panels.tex",
    data_file =
      "results/unbalanced_2month_four_cluster_active_review_trim_data.rds",
    audit_file =
      "results/unbalanced_2month_four_cluster_active_review_trim_audit.tex",
    review_seq = seq(0L, 50L, by = 5L)
) {
  ...
  writeLines(
    df_to_latex(
      ...
      label = "tab:unbalanced_2month_four_cluster_trim_audit"
    ),
    audit_file
  )
  saveRDS(
