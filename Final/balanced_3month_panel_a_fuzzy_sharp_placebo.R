...
PLACEBO_SCRIPT_NAME <- "balanced_3month_panel_a_fuzzy_sharp_placebo.R"
PLACEBO_FINAL_DIR <- if (file.exists(file.path(
  getwd(), "Final", PLACEBO_SCRIPT_NAME
))) {
  normalizePath(file.path(getwd(), "Final"), winslash = "/", mustWork = TRUE)
} else {
...
# Reuse the finalized balanced-panel construction. Its execution block is
# guarded, so sourcing it only loads data and defines helper functions.
source(file.path(
  PLACEBO_FINAL_DIR,
...
run_panel_placebo <- function(
  ...
  panel_lower <- tolower(panel)
  sample_dir <- if (review_min == 0) {
    paste0("panel_", panel_lower, "_no_review_filter")
  } else {
    paste0("panel_", panel_lower, "_review_ge_", review_min)
  }
  if (is.null(output_dir)) {
    output_dir <- file.path(OUTPUT_ROOT, sample_dir)
  }
  if (save_outputs) {
  ...
  file_prefix <- paste0("panel_", panel_lower)
  if (save_outputs) {
    ggsave(
      file.path(output_dir, paste0(file_prefix, "_1_fuzzy_rd.eps")),
      fuzzy_plot,
    ...
    ggsave(
      file.path(output_dir, paste0(file_prefix, "_2_fuzzy_stages.eps")),
      fuzzy_stages_plot,
      device = grDevices::cairo_ps,
      width = 10,
      height = 8,
    ...
    ggsave(
      file.path(output_dir, paste0(file_prefix, "_3_sharp_rd.eps")),
      sharp_plot,
      device = grDevices::cairo_ps,
      width = 10,
      height = 8,
    ...
    ggsave(
      file.path(
        output_dir,
        paste0(file_prefix, "_4_fixed_bandwidth_sharp_rd.eps")
      ),
      fixed_plot,
      device = grDevices::cairo_ps,
      width = 12,
    ...
    write_rank_tex(
      ...
      panel,
      review_min,
      file.path(output_dir, paste0(file_prefix, "_rank_table.tex"))
    )
    saveRDS(
      list(
        panel = panel,
      ...
      file.path(output_dir, paste0(file_prefix, "_placebo_results.rds"))
    )
  }
...
run_balanced_fuzzy_sharp_placebo <- function(
  ...
  for (i in seq_len(nrow(requested_samples))) {
    ...
    output[[spec$result_name]] <- run_panel_placebo(
      ...
      output_dir = file.path(output_root, spec$result_name),
      candidate_cutoffs = candidate_cutoffs,
...
if (sys.nframe() == 0L) {
  ...
  run_balanced_fuzzy_sharp_placebo(output_root = output_root)
}
