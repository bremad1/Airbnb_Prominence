...
MCCRARY_SCRIPT_NAME <- "balanced_3month_mccrary_rank.R"
MCCRARY_FINAL_DIR <- if (file.exists(file.path(
  getwd(), "Final", MCCRARY_SCRIPT_NAME
))) {
...
# Reuse the finalized balanced three-month construction, filtering, and table
# definitions. Its command-line analysis block is guarded and is not executed
# when this file sources it.
source(file.path(
  MCCRARY_FINAL_DIR,
...
run_mccrary_rank <- function(
  ...
  if (save_figures) {
    ...
    true_cutoff_tex <- c(
      ...
      "Statistic & Value \\\\",
      "\\midrule",
      sprintf("Cutoff & %.3f \\\\", true_result$cutoff),
      sprintf(
    ...
    writeLines(
      true_cutoff_tex,
      file.path(output_dir, "mccrary_4_75_table.tex")
    )
