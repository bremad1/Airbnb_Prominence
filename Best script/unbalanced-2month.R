...
#
# 2-MONTH VARIANT: avg_price (and therefore ex_price via lag) and the
# superhost-status consistency filter use only the LAST TWO calendar months
# of each quarter (months 2 and 3). Month 1 is still required to be present
# (all three months must exist for a listing-quarter to count), and month 1
# still supplies first_month_ltm / first_month_number_of_reviews and the RD
# running/treatment variables (unchanged from the 3-month version).
#
# Order of operations:
...
#      the quarterly price difference (also months 2-3 only, automatically,
#      since ex_price is just the lag of avg_price).
...
# File names are prefixed "unbalanced_2month_" so they never collide with
# unbalanced-3month.R's outputs if both are run in the same directory.
...
build_quarter_panel <- function(monthly_data) {
  ...
  quarterly <- monthly %>%
    ...
    # Within each listing-quarter, host_id must be observed and constant
    # across all three months. host_is_superhost must be observed and
    # constant across only the LAST TWO months (2-month variant) -- month 1's
    # superhost status is not checked and does not need to match. Superhost
    # status may still change between two adjacent quarters (e.g. f,f in Q4
    # followed by t,t in Q1).
    filter(
      ...
      n_distinct(host_is_superhost[.quarter_month %in% c(2L, 3L)]) == 1L
    ) %>%
    arrange(.quarter_month, Date, .by_group = TRUE) %>%
    mutate(
      # 2-month variant: avg_price uses only the last two months.
      ...
      # Carries the (already-verified-constant) months-2-3 superhost status
      # forward past the slice(1L) below, since the raw host_is_superhost
      # column on the retained month-1 row would otherwise only reflect
      # month 1's value.
      quarter_superhost_status = host_is_superhost[.quarter_month == 2L][1],
  ...
  cat(sprintf(
    "[build_quarter_panel] rows after balanced-quarter (3-month) + within-quarter host_id/2-month-superhost filter: %d\n",
    nrow(quarterly)
  ))
...
review_threshold_panel_table <- function(
  ...
  notes <- paste0(
    "avg\\_price and ex\\_price use only the last two months of each quarter; ",
    "superhost-status consistency is likewise checked over only those two ",
    "months. Before trimming, the sample is restricted to ",
    "\\texttt{first\\_month\\_ltm} $\\geq 1$ and finite price changes. ",
    "Quarter-specific trimming is then applied, followed by the displayed ",
  ...
  paste(
    c(
      ...
      "\\centering",
      sprintf(
        "\\caption{RD estimates by initial cumulative review threshold (2-month price window): Panel %s}",
        panel_letter
      ),
      sprintf(
        "\\label{tab:unbalanced_2month_review_threshold_0to50_panel_%s}",
...
run_analysis <- function(
    results_file =
      "results/unbalanced_2month_review_threshold_panels.tex",
    data_file =
      "results/unbalanced_2month_active_review_trim_data.rds",
    audit_file =
      "results/unbalanced_2month_active_review_trim_audit.tex",
    review_seq = seq(0L, 50L, by = 5L)
) {

  cat("=== [run_analysis] building unbalanced (2-month) quarterly panel ===\n")
  ...
  writeLines(
    df_to_latex(
      trim_audit,
      caption = "Quarter-level trim audit (2-month price window)",
      label = "tab:unbalanced_2month_trim_audit"
    ),
    audit_file
  )
  saveRDS(
