...
months <- data.frame(
  x = 0:9,
  label = c("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct")
)
...
# 파란 화살표 끝점 (중앙값)
green_mid_x <- mean(c(4, 6))  # May ~ Jun 중앙 (x=5)

# 빨간 화살표 + 텍스트 (Superhost selection)
selections <- data.frame(
  x = c(0, 3, 6, 9),
  label = "Superhost\nSelection",
  text_y = -1.2,
  arrow_start = -1.0,
  arrow_end = -0.35  # 월 라벨 바로 아래
)

Date_plot=ggplot() +
  # 타임라인
  geom_segment(aes(x = 0, xend = 9, y = 0, yend = 0), size = 0.8, color = "black") +
  
  # tick + 월 이름
  geom_segment(data = months, aes(x = x, xend = x, y = -0.1, yend = 0.1), size = 0.5, color = "black") +
  geom_text(data = months, aes(x = x, y = -0.3, label = label), size = 5, fontface = "bold") +
  
  # 초록 강조 구간
  geom_segment(data = segments, aes(x = xstart, xend = xend, y = 0, yend = 0),
               color = "seagreen3", size = 3, lineend = "round") +
  
  # 빨간 화살표: 아래에서 위로 (Jan, Apr, Jul, Oct 밑에 머리 위치)
  geom_segment(data = selections,
               aes(x = x, xend = x, y = arrow_start, yend = arrow_end-0.1),
               arrow = arrow(length = unit(0.12, "inches")),
               color = "firebrick2", size = 0.7) +
  
  # 빨간 텍스트: 화살표 아래
  geom_text(data = selections,
            aes(x = x, y = text_y, label = label),
            color = "firebrick2", size = 5, fontface = "italic", lineheight = 1.1, hjust = 0.5) +
  
  # 미니멀 스타일
  theme_void() +
  coord_cartesian(xlim = c(-0.5, 9.5), ylim = c(-1.5, 1)) +
  theme(plot.margin = margin(-100, 0, 0, 0)) 
Date_plot
ggsave("Figure/F1.Date.eps", plot = Date_plot,width = 10, height = 3)
ggsave("Figure/F1.Date.pdf", plot = Date_plot,width = 10, height = 4)
# Figure: Multi Score Control and Treatment Group -------------------------
arrow_stgridExtraarrow_style <- arrow(length = unit(0.2, "cm"), type = "closed")

Frontier<- ggplot() +
  # 채워진 사각형 (오른쪽)
  geom_rect(aes(xmin = 0, xmax = 0.2, ymin = -0, ymax = 0.7), fill = "grey30", color = "black", alpha = 1) +
  # 빈 사각형 (왼쪽)
  geom_rect(aes(xmin = -0.2, xmax = 0, ymin = -0, ymax = 0.7), fill = NA, color = "black") +
  # 화살표 축
  geom_segment(aes(x = -1.2, xend = 1.2, y = 0, yend = 0),
               arrow = arrow_style, color = "black") +
  geom_segment(aes(x = 0, xend = 0, y = -1.2, yend = 1.2),
               arrow = arrow_style, color = "black") +
  # 축 레이블 텍스트 추가
  annotate("text", x = 1, y = -0.1, label = "Rating", size = 4, fontface = "bold") +
  annotate("text", x = -0.22, y = 1.1, label = "Others",  size = 4, fontface = "bold") +
  # 좌표 설정 및 여백
  coord_fixed(xlim = c(-1.2, 1.2), ylim = c(-1.2, 1.2), expand = FALSE) +
  labs(title = expression("Frontier")) +
  theme_void(base_size = 13) +
  theme(
    plot.title = element_text(hjust = 0, face = "plain", margin = margin(b = 10)),
...
plot_A <- ggplot() +
  # 채워진 사각형 (오른쪽)
  geom_rect(aes(xmin = 0, xmax = 0.2, ymin = -0.1, ymax = 0.7), fill = "grey30", color = "black", alpha = 1) +
  # 빈 사각형 (왼쪽)
  geom_rect(aes(xmin = -0.2, xmax = 0, ymin = -0.1, ymax = 0.7), fill = NA, color = "black") +
  # 화살표 축
  geom_segment(aes(x = -1.2, xend = 1.2, y = 0, yend = 0),
               arrow = arrow_style, color = "black") +
  geom_segment(aes(x = 0, xend = 0, y = -1.2, yend = 1.2),
...
plot_B <- ggplot() +
  # 채워진 사각형 (오른쪽)
  geom_rect(aes(xmin = 0, xmax = 0.2, ymin = -0.7, ymax = 0.7), fill = "grey30", color = "black", alpha = 1) +
  # 빈 사각형 (왼쪽)
  
  geom_rect(aes(xmin = -0.2, xmax = 0, ymin = -0.7, ymax = 0.7), fill = NA, color = "black") +
  # 화살표 축
  geom_segment(aes(x = -1.2, xend = 1.2, y = 0, yend = 0),
               arrow = arrow_style, color = "black") +
  geom_segment(aes(x = 0, xend = 0, y = -1.2, yend = 1.2),
  ...
  # 좌표 설정 및 여백
  coord_fixed(xlim = c(-1.2, 1.2), ylim = c(-1.2, 1.2), expand = FALSE) +
  labs(title = expression("Panel B")) +
  theme_void(base_size = 13) +
  theme(
    plot.title = element_text(hjust = 0, face = "plain", margin = margin(b = 10)),
    plot.margin = margin(r=5)
  )
combined_plot <- grid.arrange(Frontier,plot_A,plot_B,ncol = 3)
ggsave("Multi-score.eps", plot = combined_plot, device = cairo_ps, width = 10, height = 4)
ggsave("Multi-score.pdf", combined_plot, width = 10, height = 4)

# 1. monthly summary table ---------------------------------------------------


# monthly_superhost_ratio_by_listings 
...
# monthly_superhost_ratio_by_hosts 
...
monthly_table <- paste(monthly_table, "\\hline \n \\end{tabular}}{\\textit{}}\n\\end{table}", sep = "")
#writeLines(monthly_table, "monthly_stats.tex")
cat(monthly_table)
# TeX 코드 출력
...
result_Q423 <- generate_quarterly_dataset(
  data = Entire,
  valid_months = c(7,8,9),
  analysis_months = c(7,8,9,10,11,12),
  target_year = 2023,
  target_months = c(10,11,12),
  cross_year = FALSE
)
...
result_Q124 <- generate_quarterly_dataset(
  data = Entire,
  valid_months = c(10,11,12),
  analysis_months = c(10,11,12,1,2,3),
  target_year = 2024,
  target_months = c(1,2,3),
  cross_year = TRUE
)
...
result_Q224 <- generate_quarterly_dataset(
  data = Entire,
  valid_months = c(1,2,3),
  analysis_months = c(1,2,3,4,5,6),
  target_year = 2024,
  target_months = c(4,5,6),
  cross_year = FALSE
)
...
save(Q323,Q323_summary_by_quartile,Q323_superhost_ratio_by_quartile,
     ...
     file = '3months quarterly dataset.RData'
)

# Aggregating superhost ratio & Superhost ratio calculation-------------------------------------------------------------
...
test_result = rddensity(z$running_scr,c=4.75)#,h=c(0.4,0.2),massPoints = T)

test_result = rddensity(z$running_scr,c=4.75,massPoints = T)
test_result = rddensity((z%>%filter(ex_super=='t'))$running_scr,c=4.75,h=c(0.3,0.15),massPoints = T,p=2)
...
plot_density_test <- rdplotdensity(rdd = test_result, type = 'both',
                                   X = z$running_scr,plotRange = c(4.2,4.85)
)
...
for (superhost in c('t','f')){
  ...
  data= data%>%filter(-results1[[1]]$bws[1,1]<= margin & margin <= results1[[1]]$bws[1,2])
  rd_plot = rdplot(y=data$price_diff,x = data$margin , subset=-results1[[1]]$bws[1,1]<= data$margin & data$margin <= results1[[1]]$bws[1,2], 
                   kernel="tri", p=1, scale=1,
                   title="RD Plot: Airbnb", 
                   y.label="",
                   x.label="Rating for Host",masspoints = 'off')
  rd_plot_first = rdplot(y=data$host_is_superhost2,,x = data$margin , subset=-results1[[1]]$bws[1,1]<= data$margin & data$margin <= results1[[1]]$bws[1,2], 
                         kernel="tri", p=1, scale=1,
                         title="", 
                         y.label="",
                         x.label="Rating for Host",masspoints = 'off')
  rd_plot
  # 왼쪽, 오른쪽 데이터 나누기
  left_data <- data %>% filter(margin >= -results1[[1]]$bws[1,1], margin < 0)
  right_data <- data %>% filter(margin >= 0, margin <= results1[[1]]$bws[1,2])
  ...
  plot_final <- ggplot() +
    ...
    theme_classic() +
    theme(
      plot.subtitle = element_text(face = "plain", hjust = 0, size = 13, margin = margin(t = 0, b = 10)),
